...
parser.add_argument('--data_dir', default=r'D:\Experiments\Vaihingen\data\test/', type=str)
...
###############################################################################
parser.add_argument('--tag', default='building_vaihingen_FlipCAM_alpha=0.25_seg_resnet101_DeepLabv3+', type=str)
...
if __name__ == '__main__':
    ...
    # pred_dir_bg = create_directory('./experiments/predictions/{}_bg/'.format(args.tag))
    # pred_dir_fg = create_directory('./experiments/predictions/{}_fg/'.format(args.tag))
    ...
    with torch.no_grad():
        ...
        for step, (ori_image, image_id, gt_mask) in enumerate(dataset):
            ...
            ori_w = ori_image.shape[0]
            ori_h = ori_image.shape[1]
            cams_list = []

            for scale in scales:
                image = copy.deepcopy(ori_image)
                image = cv2.resize(image,(round(ori_w*scale), round(ori_h*scale)))
            ...
            # preds_255 = preds * 255
            # preds_bg = preds_255[:,:,0]
            # preds_fg = preds_255[:,:,1]
            # imageio.imwrite(pred_dir_bg + image_id + '.png', preds_bg.astype(np.uint8))##
            # imageio.imwrite(pred_dir_fg + image_id + '.png', preds_fg.astype(np.uint8))##
